function [Ain, Cin, bin, fin, centerin, res] = initialize_components_w3(Y, K, options, P)
%% init for widefield 3d imaging data 
...
%Input:
%Y          d1 x d2 x d3 x T movie, raw data
%K          number of neurons to extract 
%tau        standard deviation of neuron size 
...
%
% Written by Yuichiro Hayashi 2021 with modifications from Eftychios A.
% Pnevmatikakis and Pengchen Zhou (github.com/flatironinstitute/CaImAn-MATLAB/blob/master/initialize_components.m)
% 

defoptions = CNMFSetParms;

if ~isfield(options,'init_method'); options.init_method = 'greedy'; end% ƒfƒtƒH‚Ígreedy‚¾‚ª‚ ‚Ü‚è‚æ‚­‚È‚¢
...
if options.noise_norm
    fprintf('Calc noise norm and normalized with the value\n');
    mY = mean(Y,ndims(Y));
...
elseif strcmpi(options.init_method,'greedy_corr3d')
    fprintf('Initializing components with greedy_corr_3D method \n');
    [Ain, Cin, bin, fin, centerin, res] = greedyROI_w3(Y_ds, K, options);
else
